...
> 最后更新：2026-03-09  
> 当前版本：阶段十二（均衡排产优化 + 二加求解优先级 + 剪贴板导入）
...
### 我的方案页面优化
- [x] 移除所有新建方案功能（新建按钮、创建弹窗、快速新建卡片）
- [x] 设计器方案归类为"生产方案"（统一标签）
- [x] 增加每个方案的使用次数频率（根据生产模拟中加载次数计算，显示未使用/低频/中频/高频）

### 布局优化（第三批）
- [x] 方案设计中保存方案按钮移到顶部操作栏右侧（与进入模拟验证并排）
- [x] 我的方案卡片布局统一化（固定结构：标题→标签→时间→统计→操作，解决时间显示冲突）
- [x] 我的方案中点击方案卡片可跳转到生产模拟（"进入模拟"按钮）

### 综合优化（第四批）
- [x] 全局配置产品规格参数表格转置（ABCD 作为列标题，参数作为行）
- [x] 方案设计中移除雇佣策略和机器购买 Tab 下重复的保存按钮
- [x] 我的方案跳转到生产模拟后方案下拉框显示当前选中

### 综合优化（第五批）
- [x] 生产模拟每期布局统一化（排产前后头部保持一致，无折行）
- [x] ResourceCell 组件紧凑化（whitespace-nowrap，避免折行）

### 阶段八至十一：求解算法与雇佣策略
- [x] UI 优化（首页标题、侧边栏动画、页面切换闪烁修复）
- [x] 独立求解算法 solver.ts（分层贪心 + 迭代修正）
- [x] 雇佣策略模式重构（5种模式 + 解雇固定最低）
- [x] 三档求解算法（均衡排产/极限利用/智能混合）

### 阶段十二：均衡排产优化 + 二加求解优先级 + 剪贴板导入
- [x] 均衡排产优化：每期 A/B 和 C/D 产品对自动均衡微调（balanceProductPairs）
- [x] 算法详情描述更新：三个算法的 detail 和 pros 增加均衡排产和二加优先级说明
- [x] 二加产量求解优先级调整：机器系数优先最小，其次可用人数最小
- [x] 产品规格参数剪贴板导入：支持 Tab/空格/逗号分隔、带标题/纯数字等多种格式
- [x] 剪贴板导入 Dialog 弹窗：一键粘贴 + 手动输入 + 实时预览 + 错误提示
