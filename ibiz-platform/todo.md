...
### 阶段六：方案设计器重构
- [x] 方案设计器数据类型定义（designerTypes.ts）
  - [x] 产量行为模式（必选/可选/留空/固定）
  - [x] 雇佣策略模式（最大雇佣/最少解雇/平衡/不雇佣/自定义）
  - [x] 机器购买模式（不买/固定/范围）
  - [x] 完整方案结构（DesignPlan）
- [x] 产量配置 Tab
  - [x] 1-8 期切换（左右翻页）
  - [x] 4 班次 × 4 产品配置表格
  - [x] 行为模式下拉选择（必选/可选/留空/固定）
  - [x] 参数设置（可选：范围 min~max；固定：指定值）
  - [x] 规则颜色标记（与 rule.xls 一致）
  - [x] 行为模式统计
  - [x] 复制到所有期
  - [x] 从规则表初始化
- [x] 雇佣策略 Tab
...
### 阶段七：方案设计器体验优化
- [x] 刷新保持缓存配置（localStorage 持久化当前编辑中的方案状态）
- [x] 保存方案按钮移至复制所有期下方（方便操作）
- [x] 固定行为模式规则显示绿色"固"标记
- [x] 已保存方案同步到我的方案并支持编辑跳转（仅限自己设计的方案）

### 阶段七补充：方案设计器体验优化（第二批）
- [x] 保存方案按钮移到复制到所有期左边（同一行）
- [x] 方案基本信息布局优化（紧凑行内布局，去掉 Card 包裹）
- [x] 保存方案根据方案名称不同可以多份保存（同名覆盖，不同名新建）

---

## 待完成
### 优先级 P0（核心功能）
- [ ] 方案设计器与模拟器联动（设计方案 → 加载到模拟器验证）
- [ ] 收入/成本/利润计算（需要市场价格数据）
- [ ] 多期联动完善（库存结转、机器折旧/购买）
