...
### 优先级 P0（核心功能）
- [ ] 生产决策计算引擎 — 将模拟器中的 Mock 数据替换为真实排产计算逻辑
  - [ ] 工人约束计算（正常班 + 加班工时 vs 可用工人）
  - [ ] 机器约束计算（各产品机器占用 vs 可用机器）
  - [ ] 产能计算（正常班产量 + 加班产量）
  - [ ] 收入/成本/利润计算
  - [ ] 多期联动（库存结转、工人变动、机器折旧）
- [ ] 方案 CRUD 功能 — 创建/保存/编辑/删除方案
  - [ ] 方案数据结构定义（TypeScript 类型）
  - [ ] 方案持久化（localStorage 或 Context）
  - [ ] 方案导入/导出（JSON 格式）
- [ ] 全局配置持久化 — 配置参数保存到 localStorage 并在各页面共享
